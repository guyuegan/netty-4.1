...
public abstract class AbstractByteBuf extends ByteBuf {
    ...
    // [neo] 所有的Buf实例共享同一个ResourceLeakDetector对象
    ...
    // [neo] discardReadBytes可以重用已读区，但是要进行字节数组的内存复制
    // 所以频繁调用会导致性能下降，需要谨慎使用
    @Override
    public ByteBuf discardReadBytes() {
        ...
        if (readerIndex != writerIndex) {
            // [neo] 还有可读字节，将还没读取的字节拷贝到已读区，并更新读写指针
            setBytes(0, this, readerIndex, writerIndex - readerIndex);
            writerIndex -= readerIndex;
            adjustMarkers(readerIndex);
            readerIndex = 0;
        } else {
            // [neo] 没有可读字节，不用拷贝，直接更新读写指针，后面直接覆盖
            ensureAccessible();
            adjustMarkers(readerIndex);
            // [neo] 其实可以直接调用clear {@link io.netty.buffer.AbstractByteBuf.clear}
    ...
    final void ensureWritable0(int minWritableBytes) {
        ...
        // [neo] 超过最大容量maxCapacity，一般是Integer.MAX, 抛出异常
        ...
        // [neo] 容量不够，但是fastWritable比要写入字节数大，增加fastWritable容量
        // 但maxFastWritableBytes()默认实现是writableBytes，显然比写入字节数小
        final int fastWritable = maxFastWritableBytes();
        int newCapacity = fastWritable >= minWritableBytes ? writerIndex + fastWritable
                : alloc().calculateNewCapacity(targetCapacity, maxCapacity);

        // Adjust to the new capacity.
        // [neo] 算好扩容值后，需要新建buf，并复制原buf的内容，设置读写指针/mark；
        // 不同子类有不同复制操作，这里是个抽象方法，让子类实现
    ...
    */
    ...
    public ByteBuf readBytes(byte[] dst, int dstIndex, int length) {
        checkReadableBytes(length);
        // [neo] 校验通过，从当前读指针开始，复制length个字节到目标byte数组
        // 由于不同子类复制操作技术实现细节不同，该方法由子类实现
    ...
    public ByteBuf writeBytes(byte[] src, int srcIndex, int length) {
        // [neo] todo 为什么在可写区小于要写入字节长度时，不像jdk一样抛出越界异常？
        // 参考：netty权威指南2 316
    ...
    public ByteBuf duplicate() {
        ensureAccessible();
        // [neo] 返回当前buf的复制对象，本体和副本共用缓冲区，但是维护独立的读写指针
    ...
    public ByteBuf slice() {
        // [neo] 对当前buf进行切片，切片后buf和原buf共用缓冲区，但是指针独立
    ...
    public ByteBuffer nioBuffer() {
        // [neo] netty ByteBuf转jdk ByteBuffer, 两者共用缓冲区，指针独立，
        // 但ByteBuffer无法感知ByteBuf动态扩容
    ...
    protected final void checkReadableBytes(int minimumReadableBytes) {
        // [neo] 1.如果要读取的长度小于0，抛出IllegalArgumentException
        // [neo] 2.如果要读取的长度大于可读长度，抛出IIndexOutOfBoundsException
