...
public class ResourceLeakDetector<T> {
    ...
    /* [neo]
        入参obj就是检测对象（一般都是ByteBuf）
    */
    @SuppressWarnings("unchecked")
    private DefaultResourceLeak track0(T obj) {
        // [neo] level静态字段
        Level level = ResourceLeakDetector.level;
        // [neo] 泄漏检测级别是DISABLE，直接返回
        if (level == Level.DISABLED) {
            return null;
        }

        /* [neo]
            level.ordinal() < Level.PARANOID.ordinal()即: SIMPLE, ADVANCED级别
            通过随机数的概率来抽样检测，samplingInterval：128
        */
    ...
    /* [neo]
        refQueue就是用来监听GC回收的"小本本"，如果不需要泄漏检测，就清空不再监听
    */
    ...
    protected boolean needReport() {
        // [neo] 日志级别不是error，用户看不到泄漏检测输出，所以就不需要报告检测结果
    ...
    private void reportLeak() {
        ...
        for (;;) {
            // [neo] 把小本本上记录的，已经GC"历史伟人"的会写日记的保镖（弱引用），逐个取出来看看有没有释放（弱引用列表没有对应的弱引用）
            DefaultResourceLeak ref = (DefaultResourceLeak) refQueue.poll();
            if (ref == null) {
                break;
            }

            // [neo] 弱引用列表allLeaks不包含当前小本本上的写日记的保镖（弱引用），则没有泄漏
            // todo: 就是ByteBuf被GC, 但是没有调用从allLeaks移除对应弱引用的方法？
            if (!ref.dispose()) {
                continue;
            }
    ...
    private static final class DefaultResourceLeak<T>
        ...
        boolean dispose() {
            clear();
            // [neo] java.util.Collections.SetFromMap#remove： { return m.remove(o) != null; }
            // 如果remove返回true, 说明allLeaks包含了当前弱引用
