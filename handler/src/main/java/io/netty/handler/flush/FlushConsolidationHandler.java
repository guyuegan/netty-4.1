...
readInProgress    00000111111111111111111110000000000000
                  00000111100000000000000000000000000000
    */
    @Override
    public void flush(ChannelHandlerContext ctx) throws Exception {
        // [neo] 根据业务线程是否复用IO线程两种情况考虑：
        // 复用情况 (不使用业务线程池)
        if (readInProgress) { // 正在读的时候 （read -> writeAndFlush）
            // If there is still a read in progress we are sure we will see a channelReadComplete(...) call. Thus
            // we only need to flush if we reach the explicitFlushAfterFlushes limit.
            /* [neo]
                每explicitFlushAfterFlushes次read,flush一次
                不足的最后会channelReadComplete会全部flush掉（flushPendingCount>0）
            */
        ...
        // 下面是不复用：异步情况
            ...
            /* [neo]
                开启consolidateWhenNoReadInProgress：优化flush
            ...
            */
            ...
            // [neo] 没有开启consolidateWhenNoReadInProgress，直接flush
            flushNow(ctx);
        }
    }
    ...
    // [neo] 尽快，但是还是给个优化机会（task调度有延迟，这段时间可以再积累一些写）
