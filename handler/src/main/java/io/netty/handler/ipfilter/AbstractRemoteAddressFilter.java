...
public abstract class AbstractRemoteAddressFilter<T extends SocketAddress> extends ChannelInboundHandlerAdapter {
    ...
    public void channelRegistered(ChannelHandlerContext ctx) throws Exception {
        // [neo] channel注册时，调用handleNewChannel
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        // [neo] channel激活时，调用handleNewChannel
    ...
    private boolean handleNewChannel(ChannelHandlerContext ctx) throws Exception {
        ...
        // [neo] 移除当前handler, 后续用不到了
        ctx.pipeline().remove(this);

        // [neo] accept模版方法，有三个实现：UniqueIpFilter，RuleBasedIpFilter
        if (accept(ctx, remoteAddress)) {
            // [neo] 空实现，接收channel了当前handler啥也不干；为啥留空？方便扩展？
            channelAccepted(ctx, remoteAddress);
        } else {
            // [neo] channelRejected的实现时return null, 所以走else分支，关闭掉channel
