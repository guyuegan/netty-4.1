...
public class LengthFieldBasedFrameDecoder extends ByteToMessageDecoder {
    ...
    /* [neo]
        基于长度字段的帧解码器比较重要的四个字段（示例见⬆️注释）
        lengthFieldOffset：长度字段起点偏移量
        lengthFieldLength：长度字段的字节长度
        lengthAdjustment：当长度字段保存的是整个消息（lengthField+body）长度，
                         通过该字段说明:要减去的lengthField所占字节数，计算出body字节长度
        initialBytesToStrip：去掉头几个字节，用于只保留body的情况
    */
